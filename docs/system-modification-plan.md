...
- [x] **任務 6.1**：在 `backend/tests/` 建立針對 `backtest_engine.py` 的防禦性測試，傳入特定的 Mock OHLCV，斷言「今日的訊號絕對不包含明天的資訊」。
- [x] **任務 6.2**：為 `.env` 變數加入嚴格啟動校驗，確保在啟動 `main.py` 之前必定攔截缺失的 Token 配置。
...
恭喜完成第一輪大重構！目前 **Phase 4 與 Phase 6 已完成**，系統已把前端剩餘大型元件拆分完成，並補上回測前視偏誤與啟動前環境檢查。**Phase 5 先暫停**，待券商 API 申請完成後再恢復製作。
