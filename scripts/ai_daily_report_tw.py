...
def _candidate_reason(item: dict) -> str:
    ...
    chip = ap.get("chip") or {}
    reasons: list[str] = []
    if item.get("accumulation_score") is not None:
        reasons.append(f"潛伏分數{_fmt_int(item.get('accumulation_score'))}")
    if cp.get("summary"):
...
def _sector_rotation_rows(candidates: list[dict], *, limit: int = 8) -> list[dict]:
    ...
    for sector, items in buckets.items():
        if len(items) < 2:
            continue
        acc_values = [float(item.get("accumulation_score") or 0) for item in items]
...
def _candidate_table_lines(title: str, candidates: list[dict]) -> list[str]:
    ...
    if not candidates:
        ...
        return lines
    lines.append(
        "| 類型 | 代號 | 名稱 | 族群/產業 | 潛伏分數 | K線分數 | AI篩選說明 | K線判讀 | 籌碼重點 | 新聞/事件 | 隔日策略 |"
    )
    lines.append("|---|---|---|---|---:|---:|---|---|---|---|---|")
    for it in candidates:
...
def build_report(*, base_url: str, report_date: str) -> str:
    ...
    lines.append("## 4) 可能轉強族群")
    lines.append("| 族群 | 轉強分數 | 候選數 | 平均潛伏分數 | 平均K線分數 | 法人/外資偏多數 | 突破/轉強型態數 | 量能放大數 | 代表標的 | 觀察重點 |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---|---|")
    if not sector_rows:
    ...
    lines.extend(_candidate_table_lines("## 5) 個股潛伏起漲候選（Top 20）", selected_stocks))
    lines.extend(_candidate_table_lines("## 6) ETF / 基金 / REIT 候選（Top 10）", selected_etfs))

    lines.append("## 7) 新聞與事件雷達")
